  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (id) {
          const resPedido = await axios.get(`/pedido/${id}`);
          if (resPedido.data) {
            const totalDeDB = parseFloat(resPedido.data.total);
            if (isNaN(totalDeDB) || totalDeDB <= 0) {
                setOrderError(true);
                setMontoTotal(0);
            } else {
                setMontoTotal(totalDeDB);
            }
          } else {
            setOrderError(true);
          }
    ...
    fetchData();
      ...
      const handlePayment = async () => {
        if (montoTotal <= 0 || orderError) {
          alert("No se puede procesar el cobro.");
          return;
        }

        // Ajustado según los errores de validación de tu NestJS:
        const dataFactura = { 
          id_metodo: String(metodoSeleccionado), // Debe ser string (id de Mongo)
          total: Number(montoTotal),              // Debe ser número
          id_pedido: String(id),                 // El backend lo pide como 'id_pedido' y tipo string
        };

        console.log("Enviando datos finales:", dataFactura);

        try {
          await axios.post('/factura', dataFactura);
          setPaymentCompleted(true);
        } catch (error: any) {
          const mensajesError = error.response?.data?.message;
          console.error("ERRORES DE VALIDACIÓN:", mensajesError);
          
          if (Array.isArray(mensajesError)) {
            alert(`Error de validación: ${mensajesError.join(", ")}`);
          } else {
            alert("Error al registrar factura. Revisa la consola.");
          }
        }
      };
            ...
            <div className="bg-white border-4 border-black p-8 mb-8" style={{ boxShadow: '6px 6px 0px #000000' }}>
              <div className="flex justify-between items-start mb-8 pb-6 border-b-4 border-black">
                ...
                <div className="text-right text-sm font-bold">
                  <p>N°: {invoiceNumber}</p>
                  <p>{currentDate}</p>
                  <p className="text-[#E53935]">ORDEN: #{id}</p>
                </div>
              </div>
      ...
      <div className="bg-[#263238] border-b-4 border-black" style={{ boxShadow: '0px 4px 0px #000000' }}>
        <div className="container mx-auto px-4 py-6 flex justify-between items-center">
          <div className="flex items-center gap-4">
            <button onClick={() => navigate(-1)} className="w-12 h-12 bg-[#FBC02D] border-4 border-black flex items-center justify-center hover:translate-y-1 transition-all" style={{ boxShadow: '4px 4px 0px #000000' }}>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            {tipoSeleccionado.includes('tarjeta') && !orderError && (
            ...
            <div className="border-4 border-black bg-white sticky top-8" style={{ boxShadow: '12px 12px 0px #000000' }}>
              ...
              <div className="p-6 space-y-6">
                ...
                <button 
                  ...
                  disabled={orderError || montoTotal <= 0}
                  className={`w-full bg-[#E53935] hover:bg-[#C62828] text-white border-4 border-black py-6 text-xl font-black uppercase italic flex items-center justify-center hover:translate-x-1 hover:translate-y-1 transition-all disabled:opacity-50 disabled:bg-gray-400 disabled:cursor-not-allowed`}
                  style={{ boxShadow: orderError ? 'none' : '6px 6px 0px #000000' }}
                >
                  <CheckCircle className="mr-2 h-6 w-6" /> Confirmar
                </button>
                <div className="text-center">
                    <p className="text-xs font-black uppercase text-[#263238]">Orden #{id || 'N/A'}</p>
                </div>
              </div>
            </div>
          </div>
